...
function event_info_box_content( $post ) {    
    ...
    $event_date_time = get_post_meta($post->ID, 'event_date', true);
    $event_time = get_post_meta($post->ID, 'event_time', true);
    $event_end_date_time = get_post_meta($post->ID, 'event_end_date_time', true);
    
    $container_style = "margin-bottom: 10px;";
    $label_style = "display: block; margin-right: 10px; font-weight: 900;";
    //$input_style = "width: 100%;";
    $input_style = 'width: auto;';
    ...
    // Date input
    $event_date = $event_date_time ? $event_date_time->format('F j, Y') : '';
    //$event_date = '';
    ...
    echo '<label id="event-time-label" for="event-time" style="' . $label_style . '">Start Time:</label>';
    echo '<input type="time" id="event-time-input" name="event-time" value="' . $event_time . '" placeholder="Enter a Time" style="' . $input_style . '"/>';
    echo '</div>';

    // End Time input
    $event_end_time = $event_end_date_time ? $event_end_date_time->format('H:i') : '';
    echo '<div class="event-info-input-container" id="event-time-container" style="' . $container_style . '">';
    echo '<label id="event-end-time-label" for="event-end-time" style="' . $label_style . '">End Time:</label>';
    echo '<input type="time" id="event-end-time-input" name="event-end-time" value="' . $event_end_time . '" placeholder="Enter a Time" style="' . $input_style . '"/>';
    echo '</div>';
...
function event_info_box_save( $post_id, $post ) {
    ...
    $new_event_end_time = ( isset( $_POST['event-end-time'] ) ? $_POST['event-end-time'] : '' );

    /* Get the meta key. */
    $location_meta_key = 'event_location';
    $date_meta_key = 'event_date';
    $end_time_meta_key = 'event_end_date_time';
    //$time_meta_key = 'event_time';

    /* Get the existing meta values */
    $location_meta_value = get_post_meta( $post_id, $location_meta_key, true );
    $date_meta_value = get_post_meta( $post_id, $date_meta_key, true );
    $end_time_meta_value = get_post_meta( $post_id, $end_time_meta_key, true );
    ...
    // Set EndDateTime object based on date and time input
    $new_event_end_date_time = '';
    // Both date and time were entered
    if ($new_event_date && $new_event_end_time) {
        $new_event_end_date_time = DateTime::createFromFormat('F j, Y, H:i', $new_event_date . ', ' . $new_event_end_time);
    // Only date was entered, no time
    } else if ($new_event_date) {
        $new_event_end_date_time = DateTime::createFromFormat('F j, Y, H:i', $new_event_date . ', ' . '');
    }
    
    // Update Date metadata
    // New date entered and no existing date
    if ( $new_event_date_time && '' == $date_meta_value ) {
        //add_post_meta( $post_id, $date_meta_key, $new_event_date_time, true );
        update_post_meta( $post_id, $date_meta_key, $new_event_date_time );
    }
    // New date entered differs from existing date
    elseif ( $new_event_date_time && $new_event_date_time != $date_meta_value ) {
        update_post_meta( $post_id, $date_meta_key, $new_event_date_time );
    }
    // Removing existing date
    elseif ( '' == $new_event_date_time && $date_meta_value ) {
        delete_post_meta( $post_id, $date_meta_key, $date_meta_value );
    }

      // Update END Datetime metadata
    // New enddatetime entered and no existing enddatetime
    if ( $new_event_end_date_time && '' == $end_time_meta_value ) {
        //add_post_meta( $post_id, $date_meta_key, $new_event_date_time, true );
        update_post_meta( $post_id, $end_time_meta_key, $new_event_end_date_time );
    }
    // New date entered differs from existing date
    elseif ( $new_event_end_date_time && $new_event_end_date_time != $end_time_meta_value ) {
        update_post_meta( $post_id, $end_time_meta_key, $new_event_end_date_time );
    }
    // Removing existing date
    elseif ( '' == $new_event_end_date_time && $end_time_meta_value ) {
        delete_post_meta( $post_id, $end_time_meta_key, $end_time_meta_value );
    }
