...
if ( ! function_exists( 'cpotheme_setup' ) ) {
	...
	function cpotheme_setup() {
		...
		if ( ! $epsilon ) {
			$req_plugins         = get_option( 'allegiant_show_recommended_plugins' );
			...
			if ( is_array( $req_plugins ) ) {
				...
				update_option( 'allegiant_show_recommended_plugins', $updated_req_plugins );
			}
...
if ( ! function_exists( 'cpotheme_scripts_front' ) ) {
	...
	function cpotheme_scripts_front() {
		...
		switch ( $slider_effect ) {
			case 'flipHorz':
			case 'flipVert':
				wp_enqueue_script( 'cycle2-flip', $scripts_path . 'cycle2-flip.min.js', array(
					'jquery',
					'cpotheme_cycle',
				), false, true );
				break;
			case 'tileSlide':
			case 'tileBlind':
				wp_enqueue_script( 'cycle2-tile', $scripts_path . 'cycle2-tile.min.js', array(
					'jquery',
					'cpotheme_cycle',
				), false, true );
				break;
			case 'shuffle':
				wp_enqueue_script( 'cycle2-shuffle', $scripts_path . 'cycle2-shuffle.min.js', array(
					'jquery',
					'cpotheme_cycle',
				), false, true );
				break;
...
if ( ! function_exists( 'cpotheme_add_styles' ) ) {
	...
	function cpotheme_add_styles() {
		...
		//Common styles
		wp_enqueue_style( 'cpotheme-base', $stylesheets_path . 'base.css', array(), $allegiant['Version'] );
		wp_register_style( 'cpotheme-prettyphoto', $stylesheets_path . 'prettyphoto.css', array(), $allegiant['Version'] );
		wp_register_style( 'cpotheme-magnific', $stylesheets_path . 'magnific.css', array(), $allegiant['Version'] );
		wp_enqueue_style( 'cpotheme-main', get_stylesheet_uri(), array(), $allegiant['Version'] );

		//Font Libraries
		wp_register_style( 'cpotheme-fontawesome', $stylesheets_path . 'icon-fontawesome.css', array(), $allegiant['Version'] );
		wp_register_style( 'cpotheme-linearicons', $stylesheets_path . 'icon-linearicons.css', array(), $allegiant['Version'] );
		wp_register_style( 'cpotheme-typicons', $stylesheets_path . 'icon-typicons.css', array(), $allegiant['Version'] );
	}
}
...
if ( ! function_exists( 'cpotheme_add_admin_styles' ) ) {
	...
	function cpotheme_add_admin_styles() {
		...
		wp_enqueue_style( 'select2-css', $stylesheets_path . 'select2.min.css' );
