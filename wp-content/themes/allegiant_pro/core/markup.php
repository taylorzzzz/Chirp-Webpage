...
function cpotheme_body_class( $body_classes = '' ) {
	...
	// Sticky on/off
	$sticky = cpotheme_get_option( 'sticky_header' );
	if ( $sticky == 1 ) {
			...
			foreach ( $links as $current_link ) {
				if ( $current_link['url'] != '' ) {
					...
					if ( $current_link['icon'] != '' ) {
						echo '<span class="social-icon">' . cpotheme_icon( $current_link['icon'] ) . '</span>';
					}
				...
				if ( $link_url != '' ) {
					foreach ( $social_networks as $current_network_id => $current_network_data ) {
						if ( strpos( $link_url, $current_network_id ) !== false ) {
							...
							echo '<span class="social-icon">' . cpotheme_icon( $current_network_data['icon'] ) . '</span>';
...
if ( ! function_exists( 'cpotheme_footer' ) ) {
	function cpotheme_footer() {
		...
		if ( cpotheme_get_option( 'footer_text' ) != '' ) {
			echo '<span class="copyright">' . do_shortcode( stripslashes( html_entity_decode( cpotheme_get_option( 'footer_text' ) ) ) ) . '</span>';
		} else {
